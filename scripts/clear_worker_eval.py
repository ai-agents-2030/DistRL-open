...
def move_logs(worker_ips, worker_username, worker_temp_path, wandb_run_name):
    """Kill existing processes on workers"""
    worker_log_home = os.path.split(worker_temp_path)[0]
    worker_target_log_path = os.path.join(worker_log_home, wandb_run_name)
    for worker_ip in worker_ips:
...
def main(config: "DictConfig"):
    ...
    try:
        move_logs(config.worker_ips, config.worker_username, config.worker_temp_path, config.wandb_run_name)
    except Exception as err:
